  ...
  customElements.define(
    ...
    class ProductInfo extends HTMLElement {
      ...
      handleOptionValueChange({ data: { event, target, selectedOptionValues } }) {
        console.log('### ProductInfo.handleOptionValueChange', event, target, selectedOptionValues);
      ...
      handleVariantDetails(variant) {
        const variantDetailsSelector = document.querySelectorAll(
          `#shopify-section-${this.dataset.section} .variant-details`
        );
      ...
      handlePreOrder(variant) {
        var currency =
          typeof window.variantStrings !== "undefined" && window.variantStrings.currency
            ? window.variantStrings.currency
            : "$";
        ...
        if (preOrderSelector) {
          ...
          if (stickybarCTA) {
            stickybarCTA.innerHTML = preOrder
              ? `PRE-ORDER - <span class="price-item">${currency}${variantPrice}</span>`
              : `ADD TO CART - <span class="price-item">${currency}${variantPrice}</span>`;
          }
        }
      }
      ...
      updateCtaContainer(html) {
        ...
        if (ctaContainer) {
          HTMLUpdateUtility.viewTransition(this.ctaContainer, ctaContainer, this.preProcessHtmlCallbacks);
        }
      }
      ...
      updateStickyBar(html) {
        ...
        if (stickyBar) {
          HTMLUpdateUtility.viewTransition(this.stickyBar, stickyBar, this.preProcessHtmlCallbacks);
        }
      }
      ...
      updateDesktopPriceBlock(html) {
        ...
        if (desktopPriceBlock) {
          HTMLUpdateUtility.viewTransition(this.desktopPriceBlock, desktopPriceBlock, this.preProcessHtmlCallbacks);
        }
      }
        ...
        if (newLoyaltyBadges.length > 0) {
          newLoyaltyBadges.forEach((newLoyaltyBadge, index) => {
            ...
            if (existingLoyaltyBadge) {
              HTMLUpdateUtility.viewTransition(existingLoyaltyBadge, newLoyaltyBadge, this.preProcessHtmlCallbacks);
            }
          });

          const accordionLoyaltyBadges = [
            ...document.querySelectorAll(".accordion-item__content [data-loyalty-potential-credit]"),
          ];
        ...
        return (html) => {
          ...
          const updateSourceFromDestination = (id, shouldHide = (source) => false) => {
            try {
              ...
              const destinations = this.querySelectorAll(`#${id}-${this.dataset.section}`);
