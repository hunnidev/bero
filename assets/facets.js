class FacetFiltersForm extends HTMLElement {
  constructor() {
    ...
    this.handleLabelKeydown = (event) => {
      ...
      // Check if the focused element is a label within this form
      const targetLabel = event.target;
      console.log(targetLabel)
      if (targetLabel.tagName !== 'LABEL' || !targetLabel.closest('form') === this.querySelector('form')) {
          // Ensure it's a label and it's inside *this specific* form instance
          return;
      }
      ...
      if (!htmlFor) {
        return; // Label isn't linked to an input via 'for'
      }
  
      // Find the input element using the form's elements collection (more reliable)
      const inputElement = this.querySelector('form').elements[htmlFor];
  
      // Proceed only if we found an associated checkbox
      if (inputElement && inputElement.type === 'checkbox') {
        // Prevent any potential default action for Enter on the label/form
        event.preventDefault();
  
        // Manually toggle the state of the checkbox
        inputElement.checked = !inputElement.checked;
  
        // --- CRITICAL STEP ---
        // Manually dispatch an 'input' event *on the checkbox itself*.
        // This mimics the natural event flow and ensures your existing
        // debounced listener on the form catches it via bubbling.
    ...
    // --- NEW LISTENER ---
    facetForm.addEventListener('keydown', this.handleLabelKeydown.bind(this));
    // --- END NEW LISTENER ---
...
function handleZoom() {
  ...
  let facetsContainer = document.querySelector('.mobile-facets__inner');

  const currentRatio = window.devicePixelRatio;

  if (currentRatio >= 1.5) {
      if (!facetsContainer.classList.contains('zoom-high')) {
        facetsContainer.classList.add('zoom-high');

        //Additional class for 400% zoom
        if (currentRatio >= 4) {
          facetsContainer.classList.add('zoom-400');
        } else {
          facetsContainer.classList.remove('zoom-400');
        }
      }
  } else {
    if (facetsContainer.classList.contains('zoom-high')) facetsContainer.classList.remove('zoom-high', 'zoom-400');
  }

 
}

// Debounce function to limit how often handleZoom runs during resize
function filtersDebounce(func, wait) {
  let timeout;
...
handleZoom(); // Initial call to set the correct state on load

window.addEventListener('resize', filtersDebounce(handleZoom, 500));
