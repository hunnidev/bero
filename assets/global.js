...
function trapFocus(container, elementToFocus = container) {
  // console.log('Tapping focus on:', container, 'Focusing:', elementToFocus);
...
class MenuDrawer extends HTMLElement {
  ...
  openMenuDrawer(summaryElement) {
    ...
    if (!summaryElement.classList.contains('mobile-facets__open-wrapper')) {
      ...
      //LOGIC FOR FACETS (FILTERS)

      let closeBtn = summaryElement.querySelector('.mobile-facets__close');
      trapFocus(this.mainDetailsToggle, closeBtn);
...
class ModalDialog extends HTMLElement {
  ...
  show(opener) {
    ...
    console.log(this)
    if (this.tagName == 'QUICK-ADD-MODAL') {
      let productInfoContainer = this.querySelector('.product__info-container');
      removeTrapFocus()
      
      setTimeout(() => {
        classicTrapFocus(productInfoContainer);
      }, 500)
    }

    if (this.classList.contains('modal-video')) {
      setTimeout(() => {
        classicTrapFocus(this.querySelector('.modal-video__content'));
      }, 500);
    }
...
class SliderComponent extends HTMLElement {
  constructor() {
    ...
    this.paginationDotsContainer = this.querySelector('.slider-pagination-dots');
    this.paginationDots = [];
  ...
  // New Method: Initialize Pagination Dots
  initPaginationDots() {
    console.log('Initializing pagination dots', this);

    // Clear any existing dots
    this.paginationDotsContainer.innerHTML = '';
    this.paginationDots = []; // Reset the dots array

    // Use totalPages calculated in initPages()
    for (let i = 0; i < this.totalPages; i++) {
      const dot = document.createElement('button');
      const dotSlide = this.sliderItems[i];
      dot.classList.add('slider-dot');
      dot.setAttribute('aria-label', `Slide ${i + 1}`);
      dot.setAttribute('data-index', i);
      dot.setAttribute('role', 'tab');
      dot.setAttribute('aria-controls', dotSlide.getAttribute('id'));
  ...
  // New Method: Navigate to a Specific Slide
  goToSlide(index) {
...
function classicTrapFocus(container) {
  if (container) {
    console.log('classicTrapFocus', container)
    const focusableElementsString = 'a[href], button, textarea, input[type="text"], input[type="radio"], input[type="checkbox"], select, input[type="checkbox"]+label, iframe';
    let focusableElements = container.querySelectorAll(focusableElementsString);
    focusableElements = Array.from(focusableElements).filter(el => !el.hasAttribute('disabled'));

    if (focusableElements.length > 0) {
      // console.log('focusableElements', focusableElements)
      let firstFocusableElement = focusableElements[0];
      let lastFocusableElement = focusableElements[focusableElements.length - 1];
      if (container.classList.contains('product__info-container')) {
        firstFocusableElement = container.querySelector('.product__info-container .quick-add__return');
      }
...
window.classicTrapFocus = classicTrapFocus;